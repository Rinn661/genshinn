...
bool Inventory::addItem(const Item& item, int quantity)
{
	...
	if(found != m_items.end()){
		if(found->item.type != item.type || found->item.stackable != item.stackable){
			return false;
		}

		if(item.stackable){
			if(quantity > std::numeric_limits<int>::max() - found->quantity){
				return false;
			}
	...
	const std::size_t neededSlots = static_cast<std::size_t>(quantity);
	if(m_items.size() > m_capacity || neededSlots > m_capacity - m_items.size()){
		return false;
	}

	m_items.reserve(m_items.size() + neededSlots);
	for(int i = 0; i < quantity; ++i){
		m_items.push_back(ItemStack{item, 1});
	}
	return true;
}
...
bool Inventory::removeItem(int itemId, int quantity)
{
	...
	for(auto it = m_items.begin(); it != m_items.end() && remaining > 0;){
		...
		const int removed = std::min(it->quantity, remaining);
		it->quantity -= removed;
		remaining -= removed;
...
const ItemStack* Inventory::findItem(int itemId) const
{
	...
	if(found == m_items.end()){return nullptr;}
	return &(*found);
}
...
const std::vector<ItemStack>& Inventory::items() const
{
	return m_items;
}
